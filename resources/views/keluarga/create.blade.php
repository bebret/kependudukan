...
@section('title', 'Tambah Keluarga Baru - SIPENDUK')
...
<div class="top-header">
    <div>
        <h1><i class="fas fa-home me-2 text-primary"></i> Tambah Keluarga Baru</h1>
        <p>Silakan daftarkan Kartu Keluarga baru di sini.</p>
    </div>
</div>
        ...
        <div class="card-custom mb-4">
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    <input type="text" name="nomor_keluarga" class="form-control form-control-dark @error('nomor_keluarga') is-invalid @enderror" maxlength="16" value="{{ old('nomor_keluarga') }}" required>
                ...
                <div class="col-md-6">
                    ...
                    <select name="kepala_keluarga_id" class="form-select form-select-dark @error('kepala_keluarga_id') is-invalid @enderror" required>
                        ...
                        @foreach($penduduks as $p)
                            <option value="{{ $p->id }}" {{ old('kepala_keluarga_id') == $p->id ? 'selected' : '' }}>{{ $p->nik }} - {{ $p->nama }}</option>
                        @endforeach
                    </select>
                    @if($penduduks->isEmpty())
                        <small class="text-warning mt-1"><i class="fas fa-exclamation-triangle"></i> Semua penduduk sudah memiliki keluarga. Silakan tambah data penduduk baru terlebih dahulu.</small>
                    @endif
                ...
                <div class="col-12">
                    ...
                    <textarea name="alamat" class="form-control form-control-dark @error('alamat') is-invalid @enderror" rows="3" required>{{ old('alamat') }}</textarea>
                    @error('alamat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-6">
                    <label class="form-label-custom">Kelurahan<span class="text-danger-custom">*</span></label>
                    <input type="text" name="kelurahan" class="form-control form-control-dark" value="{{ old('kelurahan') }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label-custom">Kecamatan<span class="text-danger-custom">*</span></label>
                    <input type="text" name="kecamatan" class="form-control form-control-dark" value="{{ old('kecamatan') }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label-custom">Kota/Kabupaten<span class="text-danger-custom">*</span></label>
                    <input type="text" name="kota" class="form-control form-control-dark" value="{{ old('kota') }}" required>
                </div>
                <div class="col-md-6">
                    <label class="form-label-custom">Provinsi<span class="text-danger-custom">*</span></label>
                    <input type="text" name="provinsi" class="form-control form-control-dark" value="{{ old('provinsi') }}" required>
                </div>
            </div>
        </div>

        <div class="d-flex justify-content-end gap-3">
            <a href="{{ route('keluarga.index') }}" class="btn btn-secondary px-4" style="border-radius: 8px;"><i class="fas fa-arrow-left"></i> Kembali</a>
            <button type="submit" class="btn btn-custom-blue px-4" style="border-radius: 8px;"><i class="fas fa-save"></i> Simpan Data</button>
        </div>
    </form>
</div>
@endsection